...
- If you want to write a guide for the handbook, add the new article to the [**`_posts` directory**](https://github.com/Vanilla-OS/handbook/tree/main/_posts).
- The filename must be in the format `YYYY-MM-DD-title.md`.
- Add your GitHub username along with the co-authors username(s) in the author field to have working links when the post is published.
- Set the layout as `article`.
- Headings must follow a proper hierarchy.
- Images must be in **WebP format** and be placed in `uploads` under the `assets` directory with a descriptive name for future references.
